...
class WishItemStatusService {
  static final WishItemStatusService _instance =
  ...
  Future<WishItemStatus> setItemStatus({
    ...
    try {
      ...
      if (statusQuery.docs.isEmpty) {
        // Criar novo status
        statusRef = _firestore.collection('wish_item_statuses').doc();
      } else {
      ...
      final createdAt =
          (result['created_at'] as Timestamp?)?.toDate() ?? DateTime.now();
  ...
  Future<Map<String, int>> getWishlistStats(String wishlistId) async {
    try {
      ...
      result['available'] =
          result['total']! - (result['reserved']! + result['purchased']!);

      return result;
    } catch (e) {
      debugPrint('❌ Error getting wishlist stats: $e');
      return {'total': 0, 'reserved': 0, 'purchased': 0, 'available': 0};
    }
  }
}
